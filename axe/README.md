...
1. URLを入力します。
2. 必要に応じてビュー、対象レベル（A/AA）、Basic認証、除外ルールを設定します。
...
- AAA βは一時停止中です。設定UIとAAAレベルボタンはコメントアウトされ、DEEP SCANはA/AA固定で実行されます。
- いずれかのスキャンでタイムアウトしたURLは画面に記録され、スキャン完了後に入力欄へ戻して再スキャンできます。
