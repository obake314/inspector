...
- 期待結果
  ...
  - TOTAL 行の達成率は `合格 / 全項目数` で算出され、未検証や該当なしを分母から差し引かない
...
## T-SCAN-77: 改善計画パネルの手動判定フィルタリング

- 手順
  1. MULTI SCAN を有効にして SCAN を実行し、改善計画が生成された状態にする
  2. 改善計画に表示されている項目の SC（例: 2.4.4）を確認する
  3. 詳細タブから該当 SC のカードを開き、ヒューマンチェックで `OK` に設定する
  4. 改善計画パネルの表示内容を確認する
- 期待結果
  - OK 設定後、改善計画パネルが自動的に再描画される（ページリロード不要）
  - OK に設定した SC のみを `relatedSc` に含む改善計画アクションが非表示になる
  - 複数 SC を持つアクション（例: relatedSc = [2.4.4, 2.4.6]）は、全 SC が OK になった場合のみ非表示になる
  - OK を解除（NG / 保留 に変更）すると対応するアクションが再表示される

## T-SCAN-78: スキャンキャッシュ無効化

- 手順
  1. 対象サイトのページを一度スキャンした後、サイト側でコンテンツを更新する
  2. 再度同じ URL で BASIC / DEEP / PLAY / EXT のいずれかを実行する
- 期待結果
  - 再スキャン結果に更新後のコンテンツが反映される（古いキャッシュが使われない）
  - サーバーログにキャッシュ無効化エラーが出ない
